---
title: 'STAN Code for Analyzing Intensive Longitudinal Data: Part I - Autogeressive
  Models'
...
Allright! Almost ready to begin. I just wanted to add that this is the code that I use. My own personal little Frankensteins. There are undoubtedly ways to make the code more efficient and elegant, and if you would like to comment on the code and suggests improvements, please do! I'll update the posts accordingly (and give full credit offcourse!). 
...
Below is my basic STAN-code for the AR(1) model. We can vectorize some stuff to make it a bit faster, but this is the "no frills" version of the code:

```{stan output.var=ARMod, eval=FALSE}
...
STAN code is very (veeeery) strucured. This takes some getting used to, but I ended up liking it (although I still curse it from time to time). For a very thorough discussion of the STAN language and block structure chech out this amazing "[intro](https://betanalpha.github.io/assets/case_studies/stan_intro.html#1_prologue)" by Michael Betancourt. I'll only describe the block and elements that are actually in this model. First up, the data block.
...
Finally, the model block. This is where you specify your full Bayesian model, the priors for your parameters and the observational model for your data. I'm using normal priors for my intercept and AR-parameter, and a half-normal prior for my SD. Note that I put a normal distribution on sigma, but since I specified it should be larger than 0 in the parameters block, STAN turns this into a half-normal distribution (did I already mention STAN is awesome?). For my outcome variable y, I specify the standard AR(1) model in which each observation is regressed on the immediate prior observation using the AR-parameter (beta). Note that I can't use the first observation on y, as there is no previous observation that I can use as a predictor for the first observation.
...
Basically, I cut my observational model up into two parts. First I specify an empty model for y, that just contains the intercept alpha (this is what I do in the first line within the first for-loop). In this first for-loop I specify a new "ghost" parameter, mu, that is real valued. It's a ghost variable in the sense that it is not really a model variable, and I only use it to conveniently add a not previously determined number of lags to my observational model (my likelihood) of y. It's function will become apparent shortly. In a separate second step I add the part of my observational model that predicts an observation using previous scores. This is what is happening in the nested for-loop. Notice how I can neatly cut a likelihood up into parts using the "+=" operator. That operator add the expression to the right of it to the likelihood that was already specified for the parameter on the left-side of the operator. This is the purpose of my ghost parameter mu! I want to add the entire expression of the AR model to the likelihood of y, but since the number of lagged term is not predetermined, I could not simple write out the code in the STAN-file. Instead, I add the intercept to the likelihood first (through the ghost variable mu), and then add as much lagged terms to the likelihood as needed/specified. The ghost variable mu can subsequently be specified as the mean of the normal density of y, and it will contain all relevant lagged effects as well as the intercept. 

Note that my model part ends with an empty line! This is because STAN always wnat the last line of the model code to be blank.

That's it for the STAN-code, let's quickly simulate some data in R and see if the code gives accurate estimates. 

## Testing the code

I tend to use cmdstan as my R-interface for STAN, but below I'll just use rstan. 
...
# Show Results: True alpha = 0, true beta = .5
...
As you can see, both model accurately estimate the parameters. 
